...
def render_sentence_list_and_player():
    ...
    with c3:
        limit = st.selectbox("표시 개수", [50, 100, 200, 500,1000,2000,3000,4000], index=1, key="result_limit")
...
def main():
    st.set_page_config(page_title="Vocabulary33000", page_icon="📘", layout="wide")
    init_db()
    ...
    with title_col:
        st.title("📘 Vocabulary33000")
    with help_col: 
        st.info( """     
        ** **
1. **목록/재생**에서 카테고리를 선택합니다.  
2. 문장을 선택한 뒤 **단어 불러오기**를 누릅니다.  
3. **1회 재생 / 10회 재생 / 카테고리 전체 재생**으로 학습합니다.    
4. 새 문장 추가/수정은 **입력/수정** 탭에서 합니다.
5. 전체재생은 mp3생성으로 조금 지연될수있습니다.
            """ 
    )
